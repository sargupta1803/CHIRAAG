...
import type { EvidenceData, RouteResponse } from '../types'

export const USE_MOCK_DATA = import.meta.env.VITE_USE_MOCK_DATA !== 'false'
const BASE_URL = import.meta.env.VITE_API_URL ?? ''

export async function getRoute(hour: number, alpha: number, unknownPolicy: string, from = 'Connaught Place', to = 'India Gate'): Promise<RouteResponse> {
  if (USE_MOCK_DATA) {
    const timeFactor = hour < 18 ? .18 : hour < 21 ? .62 : 1
    const detourFactor = Math.min(1, Math.max(.15, alpha / 20))
    const data = structuredClone(fixture) as RouteResponse
...
export async function getSegment(id: number): Promise<EvidenceData> {
